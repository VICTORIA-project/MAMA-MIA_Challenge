...
def datafold_read(datalist, basedir=""):
    ...
    tr_data = json_data.get("training", [])
    val_data = json_data.get("validation", [])

    def process_paths(data_list):
        for entry in data_list:
            for k, v in entry.items():
                if isinstance(v, list):
                    entry[k] = [os.path.join(basedir, iv) if not os.path.isabs(iv) else iv for iv in v]
                elif isinstance(v, str):
                    entry[k] = os.path.join(basedir, v) if v and not os.path.isabs(v) else v
        return data_list

    tr = process_paths(tr_data)
    val = process_paths(val_data)
