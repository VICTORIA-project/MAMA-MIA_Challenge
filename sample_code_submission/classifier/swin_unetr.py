...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        in_channels: int,
        num_classes: int,   # Classification task output
        ...
        # 🚀 **Classification Head: Adaptive Pooling + Fully Connected Layer**
        self.adaptive_pool = nn.AdaptiveAvgPool3d(1)  # Output shape: (B, C, 1, 1, 1)

        self.fc = nn.Sequential(
            nn.Flatten(),  # Convert (B, C, 1, 1, 1) → (B, C)
            nn.Linear(16 * feature_size, num_classes)  # Output classes (pCR vs No pCR)
        )
    ...
    def forward(self, x_in):
        ...
        # print(f"Input to SwinViT: {x_in.shape}")  # (B, C, H, W, D)
        hidden_states_out = self.swinViT(x_in, self.normalize)
        
        # for i, state in enumerate(hidden_states_out):
            # print(f"Hidden state {i}: {state.shape}")
        
        enc0 = self.encoder1(x_in)
        enc1 = self.encoder2(hidden_states_out[0])
        enc2 = self.encoder3(hidden_states_out[1])
        enc3 = self.encoder4(hidden_states_out[2])
        enc4 = self.encoder10(hidden_states_out[4])     # Deepest feature representation
        # print(f"Before Pooling: {enc4.shape}")  # Check shape before adaptive pooling

        # 🚀 **Apply Adaptive Pooling and FC layer for classification**
        pooled_features = self.adaptive_pool(enc4)  # (B, C, 1, 1, 1)
        # print(f"After Pooling: {pooled_features.shape}")  # Should be (B, C, 1, 1, 1)
        
        flattened_features = pooled_features.view(pooled_features.size(0), -1)
        # print(f"Flattened: {flattened_features.shape}")  # Should be (B, C)
        
        logits = self.fc(flattened_features)
        # print(f"Final Output: {logits.shape}")  # Should be (B, num_classes)
...
class SwinClassifier(nn.Module):
    def __init__(self, swin_backbone: SwinTransformer, num_classes: int):
        super().__init__()
        self.backbone = swin_backbone
        self.pool = nn.AdaptiveAvgPool3d(1)  # output: (B, C, 1, 1, 1)
